                ...
                if latest['buySignal'] and position != "BUY":
                    ...
                    try:
                        ...
                        if result[0] is None or (h_required and (hedge_result is None or hedge_result[0] is None)):
                            print(f"❌{key} | {user['user']} {SERVER} | No suitable option found for BUY signal. Main: {result[0]} Hedge: {hedge_result[0] if hedge_result else None}")
                            logging.error(f"❌{config['KEY']} | {SERVER}: No suitable option found for BUY signal. Main: {result[0]} Hedge: {hedge_result[0] if hedge_result else None}")
                            send_telegram_message(f"❌{config['KEY']} | {SERVER}: No suitable option found for BUY signal.",user['telegram_chat_id'], user['telegram_token'])

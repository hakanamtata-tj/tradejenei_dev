...
# def get_optimal_option_2026_03_14(signal, spot, nearest_price, instruments_df, config, user):
#     logging.info(f"{config['KEY']} | Finding optimal option for signal: {signal}, spot: {spot}, nearest_price: {nearest_price}, expiry: {config['EXPIRY']}")
#     print(f"{config['KEY']} | Finding optimal option for signal: {signal}, spot: {spot}, nearest_price: {nearest_price}, expiry: {config['EXPIRY']}")
#     if config['EXPIRY'] == "NEXT_WEEK":
#         days = 7
#     elif config['EXPIRY'] == "NEXT_TO_NEXT_WEEK":
#         days = 14
...
#     print(f"{config['KEY']} | Signal: {signal}, Spot: {spot}, Nearest 100 Strike: {strike}")
#     logging.info(f"{config['KEY']} | Signal: {signal}, Spot: {spot}, Nearest 100 Strike: {strike}")
...
#         if df_filtered.empty:
#             print(f"⚠️{config['KEY']} | No options found for strike {strike}{opt_type}")
#             logging.warning(f"⚠️{config['KEY']} | No options found for strike {strike}{opt_type}")
#             break
...
#         if target_options.empty:
#             print(f"❌{config['KEY']} | No options found in week {week_start.date()} for strike {strike}")
#             logging.warning(f"❌{config['KEY']} | No options found in week {week_start.date()} for strike {strike}")
#             break
...
#         else:
#             print(f"ℹ️{config['KEY']} | LTP {ltp} for {opt_symbol} is farther from target than previous best. Stopping search.")
#             logging.info(f"ℹ️{config['KEY']} | LTP {ltp} for {opt_symbol} is farther from target than previous best. Stopping search.")
#             break
...
#             print(f"✅{config['KEY']} | Best option found: {best_option}")
#             logging.info(f"✅{config['KEY']} | Best option found: {best_option}")
#             return best_option
#         else:
#             print(f"❌{config['KEY']} | Rejected: Best price {opt_ltp} too far from target {nearest_price}")
#             logging.warning(f"❌{config['KEY']} | Rejected: Best price {opt_ltp} too far from target {nearest_price}")

#     return None, None, None, None

# --- get_hedge_option with your original logic + fixes ---
# def get_hedge_option_2026_03_14(signal, spot, strike, instruments_df, config, user):
#     logging.info(f"{config['KEY']} |Finding hedge option for signal: {signal}, spot: {spot}, strike: {strike}, expiry: {config['EXPIRY']}")
#     print(f"{config['KEY']} | Finding hedge option for signal: {signal}, spot: {spot}, strike: {strike}, expiry: {config['EXPIRY']}")
...
#     if df_filtered.empty:
#         print(f"❌{config['KEY']} | No hedge options found for strike {hedge_strike}{opt_type}")
#         logging.warning(f"❌{config['KEY']} | No hedge options found for strike {hedge_strike}{opt_type}")
#         return None, None, None, None
...
#     if target_options.empty:
#         print(f"❌{config['KEY']} | No hedge options found in week {week_start.date()} for strike {hedge_strike}{opt_type}")
#         logging.warning(f"❌{config['KEY']} | No hedge options found in week {week_start.date()} for strike {hedge_strike}{opt_type}")
#         return None, None, None, None
...
#     if ltp is None:
#         print(f"❌{config['KEY']} | Failed to fetch LTP for hedge option {opt_symbol}")
#         logging.error(f"❌{config['KEY']} | Failed to fetch LTP for hedge option {opt_symbol}")
#         return None, None, None, None

#     print(f"✅{config['KEY']} | Hedge option found: {opt_symbol} | LTP: {ltp}")
#     logging.info(f"✅{config['KEY']} | Hedge option found: {opt_symbol} | LTP: {ltp}")
